package com.musicmundo.musicstorecatalog.repository;

import com.musicmundo.musicstorecatalog.model.Label;
import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;

import java.util.List;
import java.util.Optional;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertFalse;

@RunWith(SpringJUnit4ClassRunner.class)
@SpringBootTest
public class LabelRepositoryTests {

    @Autowired
    TrackRepository trackRepository;
    @Autowired
    AlbumRepository albumRepository;
    @Autowired
    ArtistRepository artistRepository;
    @Autowired
    LabelRepository labelRepository;

    @Before
    public void setUp() throws Exception {
        trackRepository.deleteAll();
        albumRepository.deleteAll();
        artistRepository.deleteAll();
        labelRepository.deleteAll();
    }

    @Test
    public void addGetDeleteLabel() {

        Label label = new Label();
        label.setName("MoTown Records");
        label.setWebsite("www.motown.com");
        labelRepository.save(label);

        Optional<Label> label1 = labelRepository.findById(label.getId());

        assertEquals(label1.get(), label);

        labelRepository.deleteById(label.getId());

        label1 = labelRepository.findById(label.getId());

        assertFalse(label1.isPresent());

    }

    @Test
    public void getAllLabels() {

        Label label = new Label();
        label.setName("MoTown Records");
        label.setWebsite("www.motown.com");
        labelRepository.save(label);

        label = new Label();
        label.setName("Bass Fever");
        label.setWebsite("www.bassfever.com");
        labelRepository.save(label);

        List<Label> lList = labelRepository.findAll();

        assertEquals(lList.size(), 2);

    }

    @Test
    public void updateLabel() {

        Label label = new Label();
        label.setName("MoTown Records");
        label.setWebsite("www.motown.com");
        labelRepository.save(label);

        label.setName("MoTown Records and Entertainment");
        label.setWebsite("www.motownentertainment.com");
        labelRepository.save(label);

        Optional<Label> label1 = labelRepository.findById(label.getId());

        assertEquals(label1.get(), label);
    }
}
